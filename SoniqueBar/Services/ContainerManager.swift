...
"""
    ...
    private func writeEnvIfNeeded(directory: String) async {
        ...
        let content = """
            ...
            \(LLMRoutingEnvVarNames.llmProvider)=ollama
            OLLAMA_HOST=http://host.docker.internal:11434
            OLLAMA_MODEL=gemma4
            OLLAMA_THINK=false
            \(LLMRoutingEnvVarNames.nvidiaFeatureEnabled)=false
            \(LLMRoutingEnvVarNames.nvidiaBaseURL)=<nvidia-base-url>
            \(LLMRoutingEnvVarNames.nvidiaModel)=<nvidia-model-placeholder>
            # \(LLMRoutingEnvVarNames.nvidiaAPIKey)=<nvidia-api-key>
            TTS_PROVIDER=piper
